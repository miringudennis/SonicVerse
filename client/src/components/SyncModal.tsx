            ...
            <div className="p-8 md:p-12">
               <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 rounded-2xl bg-blue-600/10 border border-blue-500/20 flex items-center justify-center text-blue-500 shadow-2xl shadow-blue-500/10">
                        <Zap className="w-6 h-6" />
                     </div>
                     <div>
                        <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter">Sync Archive.</h2>
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Neural Connection Interface</p>
                     </div>
                  </div>
                  <button 
                    onClick={onClose}
                    className="w-12 h-12 rounded-full bg-white/5 border border-white/5 flex items-center justify-center text-gray-500 hover:text-white transition-all hover:bg-white/10"
                  >
                    <X className="w-6 h-6" />
                  </button>
               </div>
                    ...
                    return (
                      ...
                      >
                         <div className="flex items-center gap-6">
                            ...
                            <div>
                               <h3 className="font-black text-white uppercase italic tracking-tighter text-lg">{platform.name}</h3>
                               {isLinked ? (
                                  <div className="flex items-center gap-2 mt-0.5">
                                     <CheckCircle2 className={`w-3 h-3 ${platform.color}`} />
                                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Linked as {account.username}</p>
                                  </div>
                               ) : (
                                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 mt-0.5 italic">Awaiting Connection</p>
                               )}
                            </div>
                         </div>
                         ...
                         {isLinked ? (
                            <div className="flex items-center gap-4">
                               <span className="text-[10px] font-black uppercase tracking-widest text-green-500/50">Synchronized</span>
                               <button 
                            ...
                            <button 
                              onClick={() => handleConnect(platform.id)}
                              disabled={isLoading}
                              className="px-8 py-3 bg-white text-black rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-white/5 disabled:opacity-50"
                            >
               ...
               <div className="mt-10 p-6 bg-blue-600/5 border border-blue-500/10 rounded-3xl flex items-start gap-4">
                  <Shield className="w-5 h-5 text-blue-500 shrink-0 mt-1" />
                  <p className="text-[11px] text-blue-200/40 font-medium leading-relaxed uppercase tracking-wider">
