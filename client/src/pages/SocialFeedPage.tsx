...
import { MessageSquare, Heart, Share2, Sparkles, Clock, Users, Search, Plus, Radio, ArrowLeft, Loader2 } from 'lucide-react';
...
export const SocialFeedPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-32 relative">
      ...
      {/* Floating Broadcast Button - Global Only */}
      {activeTab === 'global' && user && (
         <div className="fixed bottom-32 right-12 z-50">
            <button 
              onClick={() => setShowPostModal(true)}
              className="w-16 h-16 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-[0_0_30px_rgba(59,130,246,0.5)] hover:scale-110 hover:rotate-12 transition-all active:scale-95 group"
            >
               <Radio className="w-7 h-7 group-hover:animate-pulse" />
            </button>
         </div>
      )}
        ...
        {showPostModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
             ...
             <motion.div 
               ...
               className="relative w-full max-w-xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] p-10 shadow-2xl overflow-hidden"
             >
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-blue-600 to-purple-600" />
                <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter mb-8">Broadcast Signal</h3>
                <form onSubmit={handleSubmit}>
                   <textarea
                     className="w-full bg-transparent border-none p-0 text-xl font-medium text-white placeholder:text-gray-700 focus:outline-none focus:ring-0 resize-none mb-10"
                     placeholder="What's resonating in your sector?"
                     rows={4}
                     autoFocus
                     value={content}
                   ...
                   <div className="flex justify-end gap-4">
                      <button 
                        type="button" 
                        onClick={() => setShowPostModal(false)}
                        className="px-8 py-3 rounded-2xl text-[10px] font-black text-gray-500 uppercase tracking-widest hover:text-white transition-all"
                      >
                        Abort
                      </button>
                      <button 
                        ...
                        className="px-10 py-3 bg-white text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl disabled:opacity-20"
                      >
                        Deploy Signal
                      </button>
                   </div>
