...
export const DashboardPage = () => {
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      toast.error('Neural pathway disruption. Check link status.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        >
          ...
          <div className="relative z-10">
             <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] mb-6">System Analytics</p>
           ...
           <div>
              ...
              <div className="space-y-3">
                 <div className="flex items-center justify-between">
                    ...
                    {isSpotifyLinked ? <Shield className="w-3.5 h-3.5 text-[#1DB954]" /> : <Lock className="w-3.5 h-3.5 text-gray-800" />}
                 </div>
                 <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       <div className={`w-1.5 h-1.5 rounded-full ${isYoutubeLinked ? 'bg-[#FF0000]' : 'bg-gray-800'}`} />
                       <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">YouTube</span>
                    </div>
                    {isYoutubeLinked ? <Shield className="w-3.5 h-3.5 text-[#FF0000]" /> : <Lock className="w-3.5 h-3.5 text-gray-800" />}
                 </div>
              </div>
           </div>
           <Link to="/settings" className="text-[9px] font-black text-blue-500 uppercase tracking-widest hover:text-blue-400 transition-colors flex items-center gap-2 mt-4">
      ...
      <section className="space-y-6">
        ...
        {(!isSpotifyLinked && !isYoutubeLinked) ? (
          <div className="bg-[#0a0a0a] p-12 rounded-[2.5rem] border border-white/5 flex flex-col items-center text-center relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-2xl font-black text-white uppercase italic mb-4 tracking-tighter relative z-10">Neural Grid Offline</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
            {recommendations.map((track, idx) => (
              <motion.div 
                ...
                whileHover={{ y: -6 }}
                className="group flex flex-col bg-[#0a0a0a] p-2.5 rounded-[1.5rem] border border-white/5 hover:border-white/20 transition-all shadow-lg relative overflow-hidden"
              >
                ...
                <div className="px-1 relative z-10">
                  <h4 className="font-black text-white truncate text-[10px] tracking-tighter mb-0.5 uppercase italic group-hover:text-blue-400 transition-colors">{track.title}</h4>
